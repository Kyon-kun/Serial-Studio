...
QString AI::ContextBuilder::roleBlock()
{
  ...
  return QStringLiteral(
           ...
           "  frame_parsers     -> project.frameParser.*, "
           ...
           "persist across frames (last tableSet write wins; the default only "
           "seeds the initial value); Constants are read-only at "
       ...
       + QStringLiteral(
           ...
           "(7=FFT,8=LED,9=Plot,10=Bar,11=Gauge,12=Compass,20=Waterfall) used by "
           ...
           "  uniqueId      Opaque 32-bit handle for a dataset. Allocated from a "
           "persisted counter when the dataset is created; stable across reorders "
           "and moves, gone on delete, fresh on duplicate. TREAT AS OPAQUE -- "
           "never derive it arithmetically. The transform/"
           ...
           "  workspaceId   Always >= 1000. Auto-generated tabs: 1000 (Overview), "
           "1001 (All Data), 1002+ (per-group); user workspaces start at "
           "5000.\n"
...
QString AI::ContextBuilder::howToRecipe(const QString& task)
{
  ...
  if (task == QStringLiteral("bulk_dataset_edit"))
    return QStringLiteral(
      ...
      "     wgtMin/wgtMax, fftMin/fftMax. The long plotMin/widgetMin spellings "
      "     are not accepted on this surface -- they drop with an "
  ...
  if (task == QStringLiteral("health_check"))
    return QStringLiteral(
      ...
      "{kind:'frame_parser', language:0, code:'function parse(frame){ return "
      "frame.split(\",\"); }', inputBytesHex:'61 2c 62 2c 63', decoderMethod:0, "
      "frameDetection:2}. parse takes exactly ONE argument; the legacy "
      "parse(frame, separator) form is rejected.\n"
      "\n"
      ...
      "code:'function transmit(v){ return [1,; }'}.\n"
